...
# Test for Quic class initialization
...
@patch("json.loads")
def test_handle_stream_data_successful(
    mock_loads, quic_instance, mock_output, mock_state
):
...
def test_listen_setup(mock_QuicConfiguration, mock_asyncio, mock_serve, quic_instance):
    ...
    mock_serve.assert_called_once_with(
        host=host, port=port, configuration=mock_configuration, create_protocol=ANY
    )
...
def test_listen_starts_server(quic_instance, mock_configuration, mock_serve, mock_loop):
    ...
    mock_serve.assert_called_once_with(
        host=host, port=port, configuration=ANY, create_protocol=ANY
    )
...
def test_listen_server_exception_handling(
    quic_instance, mock_configuration, mock_serve, mock_loop, mock_server
):
    # Variables for the test
    cert_path = "/cert/path"
    key_path = "/key/path"
    ...
    with pytest.raises(Exception) as excinfo:
        quic_instance.listen(
            cert_path=cert_path, key_path=key_path, host=host, port=port
        )
