...
# Mocking the WebSocket context manager to prevent it from running indefinitely
@pytest.fixture
def mock_websocket_serve():
    mock_ws = mock.MagicMock()
    mock_ws.serve.return_value.__aenter__ = mock.MagicMock()
    mock_ws.serve.return_value.__aexit__ = mock.MagicMock()
    return mock_ws


# Tests initialization of the WebSocket with no custom arguments
def test_websocket_init(mock_output, mock_state):
    websocket = Websocket(mock_output, mock_state)
    assert websocket.output == mock_output
    assert websocket.state == mock_state
    assert websocket.top_level_arguments == {}
...
# Tests initialization of the WebSocket with an empty custom arguments
def test_websocket_init_empty_custom_args(mock_output, mock_state):
    websocket = Websocket(mock_output, mock_state, **{})
    assert websocket.output == mock_output
    assert websocket.state == mock_state
    assert websocket.top_level_arguments == {}
...
def test_receive_message_successful(mock_websocket):
    ...
    websocket_mock.recv = AsyncMock(return_value="sample_data")
    websocket_mock.remote_address = ("127.0.0.1", 8765)
    path = "/test"

    # Simulating the async call to receive_message method
    asyncio.run(mock_websocket.receive_message(websocket_mock, path))
...
def test_receive_message_failure(mock_websocket):
    ...
    websocket_mock = mock.MagicMock()
    websocket_mock.recv = AsyncMock(side_effect=Exception("Error!"))
    websocket_mock.remote_address = ("127.0.0.1", 8765)
    path = "/test"
